...
export default async function handler(
  ...
  try {
    ...
    switch (req.method) {
      case "GET":
        try {
          const usuario = await imovelDb.show(userId);
          return res.status(200).json(usuario);
        } catch (error) {
          return res.status(400).json({
            error: true,
            message: "Não foi posssível processar a requisição!",
          });
        }
      case "PUT":
        try {
          ...
          const usuario = await imovelDb.update(userId, data);
          return res.status(200).json(usuario);
        } catch (error) {
          return res.status(400).json({
            error: true,
      ...
      case "DELETE":
        try {
          const mensagem = await imovelDb.delete(userId);
